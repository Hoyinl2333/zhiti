...
python3 build_content.py \
  ...
  --output build \
  --version 2026.09.1 \
  --base-url https://server.example.com/v1/packs \
...
首次使用可执行 `ZHITI_SERVER_HOST=server.example.com ../infra/scripts/create-secrets.sh` 生成内容签名密钥。私钥和题库包不会提交 Git；请先确认你有权处理题库内容。
